...
io.on('connection', (socket) => {
    ...
    socket.on('playerConnect', (player) => {
        ...
        nextPlayerId++;
        // Add player to the players map with the assigned ID
        players.set(playerId, player);
        // console.log(players);
        // Send the player ID back to the client for assignment
        socket.emit('playerId', playerId);

        // Broadcast to all the clients that a new player has joined, along with the information of that player
        socket.broadcast.emit('newPlayer', player);
        console.log('Player connected:', player.name, 'with ID:', playerId);
        // console.log(players)
    });
  ...
  socket.on('clientPlayerUpdate', (playerData) => {
    ...
    // const playersJson = JSON.stringify(Array.from(players.entries()));

    socket.broadcast.emit('playerUpdates', {'id': playerData.id, 'x': playerData.x, 'y': playerData.y});
  });
